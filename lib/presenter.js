...
function handleSocketMessage(message){
  ...
  if(splitMessage[0] === "") splitMessage.shift();
...
routes.displayStreamExit = function(displayStreamID){
  ...
  // console.log("TRYING TO EXIT STREAM", displayStreamID, tries++);

  displayStreams[displayStreamID].exit();
...
DisplayStream.prototype.exit = function(){
  this.active = false;
...
DisplayView.prototype.renderHeader = function(){
  ...
  if(this.displayStream.active === true){
...
DisplayView.prototype.redrawFlowReset = function(){

  this.y = 0;
  var data = this.displayStream.data;

  var offset = (data.length > displayHeight) ? displayHeight : data.length;
...
process.stdout.on('resize', function() {
  ...
  columnWidth = Math.floor(numTTYcolumns/displayStreamIndex.length) - columnSpacing;
  displayHeight = numTTYrows - uiHeight;
...
process.on('exit', function(code) {
  ...
  process.stdout.cursorTo(0, numTTYrows);
  ...
  process.stdout.write("\u001b[?25h"); // make cursor visible again
